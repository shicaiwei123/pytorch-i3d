...
class I3D(torch.nn.Module):
    ...
    def forward_multi(self, x):
        ...
        for clip_idx in range(x.shape[1]):  # B, 10, 3, 3, 32, 224, 224
            ...
            for crop_idx in range(x.shape[2]):
                ...
                out, out_logits, feat = self.forward_single(clip)
    ...
    def forward(self, batch, save_feat=False):
        ...
        if batch[self.index_name].dim() == 5:
            feat_list = []
            out, out_logits, feat = self.forward_single(batch[self.index_name])
            pred = out_logits
            feat_list.append(feat)

        # 7D tensor == 3 crops/10 clips
        elif batch[self.index_name].dim() == 7:
            pred, feat_list = self.forward_multi(batch[self.index_name])
            # pred = out_logits
