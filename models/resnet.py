...
class I3Res50(nn.Module):
    ...
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=400, use_nl=False, args=None):
        self.inplanes = 64
    ...
    def forward(self, batch, save_feat=False):

        # 5D tensor == single clip
        # print(batch[self.index_name].dim())
        if batch[self.index_name].dim() == 5:
            feat_list = []
            pred, feat = self.forward_single(batch[self.index_name])
            feat = feat.view(feat.shape[0], -1)
            feat_list.append(feat.cpu().detach().numpy())
        ...
        if save_feat:
            return pred, loss_dict, feat_list
        else:
...
def i3_res50(num_classes, pretrain_dir, args=None):
    net = I3Res50(num_classes=num_classes, use_nl=False, args=args)
