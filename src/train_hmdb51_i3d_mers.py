...
parser.add_argument('--dataset', type=str, default='hmdb51_mers')
...
def train(iteration=0):
    ...
    cos_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[150,
                                                                          300])
...
pretrain_dir_rgb = "../pretrained/mers//weight_patch_0/_ckpt_E_248_I_0.pth"

net = i3dpt.I3D(51, modality=args.mode)
if args.mode == 'flow':
    net.load_state_dict(torch.load(pretrain_dir_flow))
elif args.mode == 'rgb':
    net.load_state_dict(torch.load(pretrain_dir_rgb)['net'])
else:
