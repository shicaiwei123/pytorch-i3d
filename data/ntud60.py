...
#         self.train_subject_list = [1, 2, 4, 5, 8, 9, 13, 14, 15, 16, 17, 18, 19, 25, 27, 28, 31, 34, 35, 38]
...
class NTUD60_CS(Dataset):
    def __init__(self, data_root, clip_len, mode, train=True, sample_interal=2):
        ...
        if mode == 'rgb':
            ...
            print(len(train_data_rgb), len(test_data_rgb))
    ...
    def sample(self, imgs):
        ...
        if len(imgs) > self.clip_len:
            ...
            if self.train:  # random sample
                ...
                step = np.int(len(imgs) / self.clip_len)
                a = np.random.randint(1, 1 + step, self.clip_len)
                b = np.arange(0, self.clip_len)
                c = a + b * step - 1
                for index in c:
                    imgs_new.append(imgs[index])
