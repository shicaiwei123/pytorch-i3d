...
def parse_annotations(root):
    def parse(annotation_csv):
        ...
        annotations_data = list(zip(*annotations))
        print(len(annotations_data))
        clip_labels, yt_ids, start_times, end_times, _, _ = annotations_data[0], annotations_data[1], annotations_data[
            2], annotations_data[3], annotations_data[4], annotations_data[5]
        ...
        for yt_id, start, end, label in tqdm.tqdm(zip(yt_ids, start_times, end_times, clip_labels), total=len(yt_ids)):
            ...
            # print("%s/%s/%s/" % (frame_dir, label, yt_id))
    ...
    print('%s/annotations/kinetics-400_train.csv' % root)
