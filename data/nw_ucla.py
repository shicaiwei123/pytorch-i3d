...
class NWUCLA_CV(Dataset):
    def __init__(self, data_root, split_num, clip_len, mode, discrete=False, train=True, sample_interal=2):
        '''
        ...
        # if train:
        #     self.clip_transform = util.clip_transform_nwucla('train', clip_len)
        # else:
        #     self.clip_transform = util.clip_transform_nwucla('val', clip_len)
        # self.train = train
    ...
    def sample_all(self, entry):
        ...
        if len(rgb_img_paths) > self.clip_len:
            ...
            if self.train:  # random sample
                ...
                if self.discrete:
                    step = (len(rgb_img_paths) // self.clip_len)
                    rgb_img_paths = rgb_img_paths[0:len(rgb_img_paths):step]
                    depth_img_paths = depth_img_paths[0:len(depth_img_paths):step]
                    if len(rgb_img_paths) > self.clip_len:
                        rgb_img_paths = rgb_img_paths[0:self.clip_len]
                        depth_img_paths = depth_img_paths[0:self.clip_len]
                else:

                    offset = len(rgb_img_paths) // 2 - self.clip_len // 2
                    rgb_img_paths = rgb_img_paths[offset:offset + self.clip_len]
                    depth_img_paths = depth_img_paths[offset:offset + self.clip_len]
                # print(len(rgb_img_paths), self.clip_len)
