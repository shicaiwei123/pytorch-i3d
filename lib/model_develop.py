...
def train_knowledge_distill_patch(net_dict, cost_dict, optimizer, train_loader, test_loader, args):
    ...
    while epoch < epoch_num:
        ...
        for batch_idx, (multi_sample, single_sample, label) in enumerate(
            ...
            teacher_whole_out, teacher_patch_out = teacher_model(img_rgb, img_ir, img_depth)
            ...
            if args.kd_mode in ['logits', 'st']:
                ...
                # kd_loss = mmd_loss(student_patch_out, teacher_patch_out.detach())
                if args.weight_patch:
                    kd_loss = bce_loss(student_patch_out, teacher_patch_out.detach())
    ...
    while epoch < epoch_num:
        ...
        for batch_idx, (multi_sample, single_sample, label) in enumerate(
            ...
            if args.student_data == 'multi_rgb':
                ...
                student_whole_out, student_patch_out, student_layer3, student_layer4 = student_model(single_sample)
            time_forward = datetime.datetime.now() - start
            # print("time_forward:", time_forward.total_seconds())
            ...
            kd_feature_loss = sp_loss(student_layer3, teacher_layer3)
            ...
            loss = cls_loss + kd_feature_loss * 0.1 + kd_logits_loss
...
def train_knowledge_distill_sift_patch(net_dict, cost_dict, optimizer, train_loader, test_loader, args):
    '''
    ...
    while epoch < epoch_num:
        ...
        for batch_idx, multi_sample in enumerate(
            ...
            img_rgb, img_ir, img_depth, target, cluster_center = multi_sample['image_x'], multi_sample['image_ir'], \
                                                                 multi_sample['image_depth'], multi_sample[
                                                                     'binary_label'], multi_sample[
                                                                     'cluster_center']
            ...
            if torch.cuda.is_available():
                ...
                img_depth = img_depth.cuda()
                target = target.cuda()
                cluster_center = cluster_center.cuda()

            optimizer.zero_grad()

            teacher_whole_out, teacher_patch_out = teacher_model(img_rgb, img_ir, img_depth, cluster_center)

            student_whole_out, student_patch_out = student_model(img_rgb, cluster_center)
        ...
        # testing
        result_test = calc_accuracy_kd_sift_patch(model=student_model, args=args, loader=test_loader)
        accuracy_test = result_test[0]
        if accuracy_test > accuracy_best and epoch > 10:
            accuracy_best = accuracy_test
            save_path = os.path.join(args.model_root, args.name + '.pth')
            torch.save(student_model.state_dict(), save_path, )
        log_list.append(train_loss / len(train_loader))
